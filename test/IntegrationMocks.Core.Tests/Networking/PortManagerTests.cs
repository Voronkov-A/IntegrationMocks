...
public class PortManagerTests
{
    ...
    {
        _portRepository = new DirectoryStringRepository(PortManager.DefaultPortNumberRepositoryDirectoryPath);
        _portMonitorMock = new Mock<IPortMonitor>();
        _portRange = new Range<int>(34243, 34443);
        _sut = new PortManager(_portRepository, _portMonitorMock.Object, _portRange);
    }
    ...
    public async Task TakePort_is_thread_safe()
    {
        ...
        {
            await Task.WhenAll(
                Enumerable.Range(0, 100).Select(_ => Task.Run(() => ports.Add(_sut.TakePort()))));
    ...
    public void TakePort_throws_when_all_ports_are_in_use()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => _sut.TakePort());
    }
    ...
    private static int CreatePortAndForget(IPortManager manager)
    {
        return manager.TakePort().Number;
    }
}
