...
public sealed class DirectoryPortNumberRepositoryTests : IDisposable
{
    ...
    private readonly DirectoryPortNumberRepository _sut;

    public DirectoryPortNumberRepositoryTests()
    {
        _fixture = new Fixture();
        _directoryPath = Path.Combine(
            Path.GetTempPath(),
            $"{nameof(IntegrationMocks)}_{nameof(DirectoryPortNumberRepositoryTests)}");
        if (Directory.Exists(_directoryPath))
        {
            Directory.Delete(_directoryPath, recursive: true);
        }
        _sut = new DirectoryPortNumberRepository(_directoryPath);
    }
    ...
    public void Add_creates_file()
    {
        ...
        Assert.True(added);
        Assert.True(File.Exists(Path.Combine(_directoryPath, value.ToString())));
    }
    ...
    public void Remove_deletes_previously_created_file()
    {
        ...
        Assert.False(File.Exists(Path.Combine(_directoryPath, value.ToString())));
    }
    ...
    {
        var expectedValues = _fixture.CreateMany<int>().ToHashSet();
    ...
    {
        var expectedValues = _fixture.CreateMany<int>().ToHashSet();
        var removedValues = _fixture.CreateMany<int>().ToList();
